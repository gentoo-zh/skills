...
- `gzh commit` 封装的是 `pkgdev commit --scan false --signoff --gpg-sign`：`--scan false` 是因为 `gzh pkgcheck` 已是独立硬门，不重复跑；`--gpg-sign` 只在 git 配了 `user.signingkey` 时才加，因为 `gzh commit` 只查这一条、不探测 gpg 能不能用。省略了也不等于没签名，`commit.gpgsign=true` 时 git 照签，别在报告里写成未签名。绝不用裸 `git commit`。
...
- **联网 DeadUrl 复查**：收尾跑 `gzh pkgcheck-commits`，它执行 `pkgcheck scan --git-remote <canonical> --commits="$(git merge-base <canonical>/master HEAD)..HEAD" --net`。因为裸 `--commits` 比的是 fork 那份落后的 `origin`、会把无关包一起扫进来，所以范围必须用 merge-base 显式限死。这一段 CI 与 `gzh pkgcheck` 都不跑，属 PR 勾选项，须手动补跑。
