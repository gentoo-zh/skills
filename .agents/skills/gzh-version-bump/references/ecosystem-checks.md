...
- **unresolved-soname/RPATH 被压而非修** — -bin ebuild 里的 QA_SONAME/QA_FLAGS_IGNORED 赋值（注：`QA_PREBUILT="*"` 是惯用声明，但**不** gag soname 检查）。压 QA 只掩盖 bundled 组件解析不了私有库；正解 `patchelf --set-rpath '$ORIGIN/...'`（+ 对真系统库的真 RDEPEND）。详见 [prebuilt-qa.md](prebuilt-qa.md)。 _[13a7a9d, f9a2b99]_
