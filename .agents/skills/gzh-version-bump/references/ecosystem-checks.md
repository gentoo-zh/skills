...
- **强塞全局优化/加固/LTO/`-Werror`** — grep meson.build 的 `add_global_arguments|add_project_arguments`、Makefile/Makefile.in/configure 里 `-flto -D_FORTIFY_SOURCE -fstack-protector -O[0-9s] -Werror`；未打 patch 的命中即违规，用 respect-flags patch 或 `src_prepare` 里 sed 删掉。上游硬加会盖过发行版工具链加固和用户 flag，新版会重新引入。`-Werror` 只删裸的那种：因为裸 `-Werror` 会把新编译器多出来的警告变成硬失败，所以 bump 到新工具链必挂；`-Werror=<具体项>` 是上游针对性收紧、不受新警告波及，留着不动。 _[e6dc5e5]_
...
- **go.mod go 指令 vs BDEPEND** — 取新版 go.mod 的 `go X.Y.Z`，对比 ebuild 里最高的 `>=dev-lang/go-*`；ebuild 自己没写 go atom 时，下限就是 go-module.eclass 里那条默认 BDEPEND，读树里的实际值。go.mod 高于该下限即违规，按 go.mod 抬 BDEPEND；不高于就别加冗余 atom。`toolchain` 那行不构成下限：因为树里 dev-lang/go 各版本都打了不下载更新 toolchain 的 patch、把 `GOTOOLCHAIN` 定成 `local`，所以那行不产生版本要求，别拿它抬 BDEPEND。只有实测出不兼容且无可用修法时才加上界。 _[bbdf70d]_
...
- **@PRE_INHERIT 变量写在 inherit 之后** — 加 eclass 或调整 inherit 行时，对每个 eclass 读 `$(portageq get_repo_path / gentoo)/eclass/<name>.eclass` 的头，grep `^# @PRE_INHERIT`，确认这些变量的赋值都排在 `inherit` 之前。因为 eclass 在 inherit 期间就读这些值去定 BDEPEND 和默认分支，所以设晚了 eclass 读到的是空：标 `@REQUIRED` 的当场 die（`RPM_COMPRESS_TYPE` 在 EAPI 9、`DISTUTILS_USE_PEP517`），有默认值的静默走默认（`CMAKE_MAKEFILE_GENERATOR` 空则取 ninja 并据此定 BDEPEND），后者离线检查一条都看不出来。 _[主树 eclass 头]_
- **两个 eclass 导出同一个 phase** — `inherit` 行逐个 eclass grep `EXPORT_FUNCTIONS`，同一个 phase 被两个以上导出即违规。因为 portage 在 `inherit` 里按 inherit 顺序把每个导出 phase 定义成转调 `<eclass>_<phase>` 的函数，后 inherit 的直接盖掉前一个、不 die 也不 eqawarn，所以要在 ebuild 里显式定义这个 phase 并按需要依次调 `<eclass>_<phase>`。别用 `default` 顶：它调的是 EAPI 默认实现，不是 eclass 导出的那个，eclass 行为会整个丢掉。 _[portage bin/ebuild.sh inherit()]_
- **新版新增的可选依赖被自动探测** — 新加的可选功能常按构建机上有没有那个库来决定编不编：meson 查 `meson.options`/`meson_options.txt` 里 `type: 'feature'` 的项（不写 `value` 就是 `auto`），cmake 查不带 `REQUIRED` 的 `find_package()`。ebuild 没把它钉成定值即违规——meson 用 `$(meson_feature flag opt)` 或整体 `-Dauto_features=disabled` 再逐个开，cmake 用 `$(cmake_use_find_package flag Pkg)`。因为 `auto` 的结果随构建机装了什么变，所以同一份 ebuild 在不同机器上装出的内容和实际链接的库都不一样、`RDEPEND` 也对不上，而 pkgcheck 和 CI 都看不出来。钉住时二选一：加新 `IUSE` 并配上对应依赖，或者明确 disabled 且不加依赖。 _[meson.eclass / cmake.eclass]_
