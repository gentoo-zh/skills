...
**区分包类型**：`SRC_URI` 取 `.deb`/`.rpm`/`.AppImage`/预编译 tarball，或 ebuild 设了 `QA_PREBUILT`/`RESTRICT=strip` 的，A4 之后逐节过一遍 [prebuilt-qa.md](references/prebuilt-qa.md)：`KEYWORDS` 与 per-arch `SRC_URI`、`RESTRICT` 与 license、`QA_PREBUILT` 的路径作用域、逐个 ELF 的 `NEEDED`/RPATH 与 unresolved soname 排查、`.desktop` 与 exec 位、locale/edition、多 arch。因为 `gzh pkgcheck` 是静态扫描、`gzh build-test` 的 `qa_notices` 又刻意跳过 `Unresolved soname` 留给收尾步骤 4 那道解析依赖的 emerge 门，所以预编译包最常红的那一类本地两道门都不判，只做 A4 就收等于把它留到 CI。`inherit` 含 `xdg`/`desktop` 的包不论源码还是预编译，都要核 `.desktop` 那条（在 ecosystem-checks 里），因为那道门按装出来的文件判、不看包是怎么来的。源码包重在 A5/A6，按生态对照 [ecosystem-checks.md](references/ecosystem-checks.md)（A5/A6 对照、build-test 红时倒查）。
