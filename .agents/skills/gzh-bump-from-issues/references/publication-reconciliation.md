...
procedure. Build a separate immutable `gzh pr-plan` for every branch and obtain a separate
confirmation for this one rendered pull request. Prior batch, multi-plan, or wildcard
authorization does not count. Recompute each plan before `gh pr create` or `gh pr edit`, and
